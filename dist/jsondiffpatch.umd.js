...
var BaseFormatter = function () {
  ...
  createClass(BaseFormatter, [{
    key: 'format',
    value: function format(delta, left, translationConfigMap) {
      var context = {};
...
var HtmlFormatter = function (_BaseFormatter) {
  ...
  createClass(HtmlFormatter, [{
    ...
    value: function formatValue(context, value) {
      context.out('<pre>' + htmlEscape(JSON.stringify(value, function (k, v) {
        if (typeof v === 'bigint') {
    ...
    value: function nodeBegin(context, key, leftKey, type, nodeType) {
      ...
      var newLeftKey = leftKey;
      if (this.translationConfigMap &&
      ...
      var nodeClass = 'jsondiffpatch-' + type + (nodeType ? ' jsondiffpatch-child-node-type-' + nodeType : '');
      context.out('<li class="' + nodeClass + '" data-key="' + leftKey + '">' + ('<div class="jsondiffpatch-property-name">' + newLeftKey + '</div>'));
    }
  }, {
    key: 'nodeEnd',
...
function htmlEscape(text) {
  ...
  // TODO: 处理bigint
  var bigints = html.match(/"Bigint=.*?"/g);
  if (bigints && bigints.length) {
    for (var i = 0; i < bigints.length; i++) {
      var bi = bigints[i];
      var bi1 = bi.replace('"Bigint=', '').replace('"', '');
  ...
  for (var _i = 0; _i < replacements.length; _i++) {
    html = html.replace(replacements[_i][0], replacements[_i][1]);
  }
  return html;
}
...
function format(delta, left, translationConfigMap) {
  if (!defaultInstance) {
...
var formatAnyChange = function formatAnyChange(context, delta) {
  ...
  var json = JSON.stringify(delta, function (k, v) {
    if (typeof v === 'bigint') {
      return 'Bigint=' + v.toString();
    }
    return v;
...
var ConsoleFormatter = function (_BaseFormatter) {
  ...
  createClass(ConsoleFormatter, [{
    ...
    key: 'formatValue',
    value: function formatValue(context, value) {
      context.out(JSON.stringify(value, function (k, v) {
        if (typeof v === "bigint") {
          return 'Bigint=' + v.toString();
        }
        return v;
